"""Assemble an out-of-the-box release directory for java-runner2026.

Output layout (dist/java-runner2026/):
  java-runner2026.exe
  java-runner2026.toml
  config/java-runner2026.toml
...
DIST_ROOT = ROOT / "dist" / "java-runner2026"
...
<head>
  ...
  <title>java-runner2026 发布包说明</title>
  <style>
...
<body>
  <header>
    <h1 style="margin:0 0 .5rem">java-runner2026 发布包</h1>
  ...
  <main>
    <div class="card">
      ...
      <ol class="steps">
        ...
        <li>双击 <code>java-runner2026.exe</code> 运行（程序会自动切换 UTF-8 并按 exe 所在目录加载配置）</li>
        <li>按提示选择 JVM，程序将执行 <code>java-runner2026.toml</code> 中配置的启动命令</li>
        <li>修改启动参数：编辑根目录 <code>java-runner2026.toml</code> 后重新运行</li>
      </ol>
    ...
    <div class="card">
      <h2>目录说明</h2>
      <ul>
        <li><code>java-runner2026.exe</code> — 主程序</li>
        <li><code>java-runner2026.toml</code> — 启动配置（推荐编辑此文件）</li>
...
{doc_links}
    ...
    <div class="card">
      <h2>常用命令</h2>
      <pre><code>java-runner2026.exe --scan-only
java-runner2026.exe --select 1
java-runner2026.exe --json
java-runner2026.exe --config java-runner2026.toml
java-runner2026.exe --no-pause</code></pre>
    </div>
  </main>
</body>
</html>
...
def find_exe() -> Path:
    candidates = [
        ROOT / "release" / "java-runner2026.exe",
        ROOT / "target" / "release" / "java-runner2026.exe",
        ROOT / "java-runner2026.exe",
    ]
    for path in candidates:
        if path.is_file():
            return path
    raise SystemExit(
        "未找到 java-runner2026.exe。请先执行 cargo build --release，或将 exe 放入 release/ 目录。"
    )
...
def package(dist: Path) -> None:
    ...
    shutil.copy2(exe_src, dist / "java-runner2026.exe")
    print("  java-runner2026.exe")

    config_file = CONFIG_SRC / "java-runner2026.toml"
    if not config_file.is_file():
        raise SystemExit(f"缺少配置文件: {config_file}")

    shutil.copy2(config_file, dist / "java-runner2026.toml")
    (dist / "config").mkdir(exist_ok=True)
    shutil.copy2(config_file, dist / "config" / "java-runner2026.toml")
    print("  java-runner2026.toml")
    print("  config/java-runner2026.toml")
